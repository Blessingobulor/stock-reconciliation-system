...
function needed_items($all_array, $not_needed_array){
    ...
    {
                if (in_array($key, $not_needed_array) && ($key !== "transfer_stock_id" && $key !== "date" && $key !== "destination" && $key !== "branch_name" && $key !== "stock_details" )) 
        {
            unset($all_array[$key]);
        }
    }
...
{ 
    ...
    $transfer_to_branch->insert($items);
    $success_message = 'Submitted successfully';
}
}


require_once views_path('transfer-to-branch/transfer-to-branch-new');
