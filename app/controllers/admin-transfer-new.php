...
function needed_items($all_array, $not_needed_array){
    ...
    {
                if (in_array($key, $not_needed_array) && ($key !== "transfer_stock_id" && $key !== "date" && $key !== "destination" && $key !== "branch_name" && $key !== "reasons_for_transfer" )) 
        {
            unset($all_array[$key]);
        }
    }
...
{ 
    ...
    $items = [
       ...
       "stock_name" => $needed["stock_name_" . $i],
        ...
        "reasons_for_transfer" => isset($needed["reasons_for_transfer"]) ? $needed["reasons_for_transfer"] : null,
    ];
    ...
    $admin_transfer->insert($items);
    $success_message = 'Submitted successfully';
}
}


require_once views_path('admin-transfer/admin-transfer-new');
