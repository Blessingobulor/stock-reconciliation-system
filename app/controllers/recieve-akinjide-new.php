...
function needed_items($all_array, $not_needed_array){
    ...
    {
                if (in_array($key, $not_needed_array) && ($key !== "recieve_stock_id" && $key !== "date" && $key !== "source" && $key !== "branch_name"  )) 
        {
            unset($all_array[$key]);
        }
    }
...
require_once views_path('recieve-akinjide/recieve-akinjide-new');
