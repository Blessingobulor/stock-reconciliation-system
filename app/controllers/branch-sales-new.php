...
function needed_items($all_array, $not_needed_array){
    ...
    {
                if (in_array($key, $not_needed_array) && ($key !== "sales_id" && $key !== "date"  && $key !== "branch_name" && $key != "customer_name"  && $key != "vat_amount" && $key != "grand_total" && $key != "sub_total")) 
        {
            unset($all_array[$key]);
        }
    }
...
"stock_name"=>$needed["stock_name_".$i], 
        ...
        "total"=>$needed["total_".$i],
        "sub_total"=>$needed["sub_total"],
        "vat_amount"=>$needed["vat_amount"],
        "grand_total"=>$needed["grand_total"],
        "date"=>$needed["date"],
 ...
 // Checking if the stock name already exists in the table
    ...
    $branch_sales->insert($items);
    $success_message = 'Submitted successfully';
}
}


require_once views_path('branch-sales/branch-sales-new');
