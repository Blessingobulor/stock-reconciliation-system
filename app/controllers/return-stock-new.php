...
function needed_items($all_array, $not_needed_array){
    ...
    {
                if (in_array($key, $not_needed_array) && ($key !== "return_stock_id" && $key !== "date"  && $key !== "branch_name"  && $key !== "destination"  && $key !== "reasons_for_return_of_stock")) 
        {
            unset($all_array[$key]);
        }
    }
...
{   // PUBLIC DECLARATION OF THE STOCK NAME AND QUANTITY TO BE USED FOR CHECKING AND UPDATING IF NECESSARY
    ...
    $items = [
        ...
        "return_stock_id" => isset($needed["return_stock_id"]) ? $needed["return_stock_id"] : null,
         ...
         "destination" => isset($needed["destination"]) ? $needed["destination"] : null,
    ...
    $return_stock->insert($items);
    $success_message = 'Submitted successfully';
}
}


require_once views_path('return-stock/return-stock-new');
