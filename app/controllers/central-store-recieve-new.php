...
function needed_items($all_array, $not_needed_array){
    ...
    {
                if (in_array($key, $not_needed_array) && ($key !== "recieve_stock_id" && $key !== "date" && $key !== "source" && $key !== "branch_name" && $key !== "recieved_by"  && $key !== "name_of_reciever" )) 
        {
            unset($all_array[$key]);
        }
    }
...
{ 
    ...
    $central_store_recieve->insert($items);
...
require_once views_path('central-store-recieve/central-store-recieve-new');
