...
function needed_items($all_array, $not_needed_array){
    ...
    {
                if (in_array($key, $not_needed_array) && ($key !== "recieved_stock_id" && $key !== "branch_name" && $key !== "supplier_name" && $key !== "date" && $key !== "recieved_by" )) 
        {
            unset($all_array[$key]);
        }
    }
...
{ 
    ...
    $items = [
       ...
       "stock_name" => $needed["stock_name_" . $i],
            ...
            "number_of_stock_ordered" => $needed["number_of_stock_ordered_" . $i],
    ...
    $electronics_recieved_from_supplier->insert($items);
    $success_message = 'Submitted successfully';
}
}


require_once views_path('electronics-recieved-from-supplier/electronics-recieved-from-supplier-new');
