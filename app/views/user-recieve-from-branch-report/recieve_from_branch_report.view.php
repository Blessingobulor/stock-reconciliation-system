...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_stock'])) {
   ...
   $db->query("DELETE FROM recieve_from_branch WHERE id = $id");
...
<div>
    ...
    <h2>RECIEVE FROM BRANCH REPORT</h2>
    <br><br>
    <hr>
    ...
    <a href="index.php?pg=recieve-from-branch-new">
       <button style="margin: 30px; background-color: blue; max-width: 70px; height: 40px; position: grid; top: 30px; width: 70px; border-radius: 2px;" type="button" class="btn btn-primary"> Cancel </button>
        ...
        <button style="margin: 30px; max-width: 70px; height: 40px; position: grid; top: 30px; width: 70px; border-radius: 2px;"
            class="btn btn-primary btn-sm col" name="generate_report_from_recieve_branch" type="submit"> Generate</button>
    </a>

    <input type="hidden" name="pg" value="user-recieve-from-branch-report">
    <input type="hidden" name="tab" value="recieve_from_branch_report">
         ...
         buttons: [
            {
               ...
               title: 'Recieve from Branch Report'
            },
            {
               extend: 'print',
               exportOptions: {
