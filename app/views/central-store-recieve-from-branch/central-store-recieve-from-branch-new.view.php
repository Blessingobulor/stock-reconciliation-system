...
<div class="container-fluid border col-lg-7 col-md-6 mt-4 p-3 shadow-lg rounded">
    <a href="index.php?pg=central_store_recieve_from_branch_report">
    ...
    <form id="stock-form" name="myform" method="post">
        <center>
            <br>
    <h3>
        <i class="fa fa-dolly" style="margin-right: 10px;"></i>Central Store Receive From Branch
    </h3>
        </center>

        <div class="mb-3 mt-3">
            <label for="recievestockidControlInput1" class="form-label">Recieve Stock Id</label>
                <input type="text" class="form-control" name="recieve_stock_id" id="id" value ="RS<?= rand(1000, 9999) ?>" readonly>

            <?php if(!empty($errors['return_stock_id'])):?> 
                <small class="text-danger"><?=$errors['recieve_stock_id']?></small>
            <?php endif;?>
        </div>
        ...
        <div class="mb-3 mt-3">
            <label for="dateControlInput1" class="form-label">Recieve Date</label>
         ...
         <div class="mb-3 mt-3">
            <label for="reasonsforrecievingofstockControlInput1" class="form-label">Reasons For Recieve</label>
            <select name="reasons_for_recieving_of_stock" class="form-control <?=!empty($errors['reasons_for_recieving_of_stock']) ? 'border-danger' : ''?>" id="reasonsforrecievingofstockControlInput1" required>
                <option value="" hidden>Select Reasons...</option>
                <option value="damage stock">damage stock</option>
                <option value="incomplete stock">incomplete Stock</option>
                <option value="old stock">old stock</option>
            </select>
            <?php if(!empty($errors['reasons_for_recieving_of_stock'])):?>
                <small class="text-danger"><?=$errors['reasons_for_recieving_of_stock']?></small>
            <?php endif;?>
        </div>
       ...
       <h5>Add Stock Recieved</h5>
        ...
        <input name="stock_name_0" class="js-search form-control <?=!empty($errors['qty']) ? 'border-danger' : ''?>" oninput="searchItem(event)" placeholder="Stock Recieved" required>
...
<script>
    ...
    function AddMore() {
        ...
        stock_name.placeholder = "Stock Recieved";
