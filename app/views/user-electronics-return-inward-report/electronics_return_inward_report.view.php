...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_stock'])) {
    ...
    // Delete the stock item from the cctv_return_inward table

   $db->query("DELETE FROM electronics_return_inward WHERE id = $id");
...
<div>
    ...
    <form style="margin: 40px;"class="row float-end <?= !Auth::access('cashier') ?> ">
            <h2>ELECTRONICS RETURN INWARD REPORT</h2>
        ...
        <br><br>
                ...
                <a href="index.php?pg=electronics-return-inward-new">
            ...
            class="btn btn-primary btn-sm col" name="generate_report_electronics_return_inward" type="submit"> Generate</button>

            </a>

        <input type="hidden" name="pg" value="user-electronics-return-inward-report">
        <input type="hidden" name="tab" value="electronics_return_inward_report">
...
<div class="table-responsive">
    ...
    <?php foreach($report as $report): ?>
        <tr>
            <td><?= $report['return_stock_id'] ?></td>
            <td><?= $report['branch_name'] ?></td>
            <td><?= $report['customer_name'] ?></td>
            <td><?= date('j M, Y', strtotime($report['date'])) ?></td>
            <td><?= $report['reasons_for_return_of_stock'] ?></td>
         ...
         buttons: [
            {
               ...
               title: 'Electronics Customer Return Inward Report'
            },
            {
               extend: 'print',
               exportOptions: {
