...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_stock'])) {
   ...
   $db->query("DELETE FROM electronics_recieved_from_supplier WHERE id = $id");
...
<div>
    ...
    <h2>Electronics Recieved From Supplier Report</h2>
    <br><br>
    <hr>
    ...
    <a href="index.php?pg=electronics-recieved-from-supplier-new">
       <button style="margin: 30px; background-color: blue; max-width: 70px; height: 40px; position: grid; top: 30px; width: 70px; border-radius: 2px;" type="button" class="btn btn-primary"> Cancel </button>
        ...
        <button style="margin: 30px; max-width: 70px; height: 40px; position: grid; top: 30px; width: 70px; border-radius: 2px;"
            class="btn btn-primary btn-sm col" name="generate_report_electronics_recieved_supplier" type="submit"> Generate</button>
    </a>

    <!-- <input type="hidden" name="pg" value="users"> -->
    <input type="hidden" name="pg" value="user-electronics-recieved-from-supplier-report">
    <input type="hidden" name="tab" value="electronics_recieved_from_supplier_report">
...
<div class="table-responsive" id="electronics_recieved_from_supplier_report">
