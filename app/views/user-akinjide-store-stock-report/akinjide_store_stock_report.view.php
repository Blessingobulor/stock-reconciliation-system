...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_stock'])) {
   ...
   $db->query("DELETE FROM akinjide_store_stock WHERE id = $id");


    // update the stock quantity in the stock table after the delete
...
<div>
    ...
    <a href="index.php?pg=akinjide-store-stock-new">
       <button style="margin: 30px; background-color: blue; max-width: 70px; height: 40px; position: grid; top: 30px; width: 70px; border-radius: 2px;" type="button" class="btn btn-primary"> Cancel </button>
        ...
        <button style="margin: 30px; max-width: 70px; height: 40px; position: grid; top: 30px; width: 70px; border-radius: 2px;"
            class="btn btn-primary btn-sm col" name="generate_report_akinjide_store_stock" type="submit"> Generate</button>
    </a>

    <!-- <input type="hidden" name="pg" value="users"> -->
    <input type="hidden" name="pg" value="user-akinjide-store-stock-report">
    <input type="hidden" name="tab" value="akinjide_store_stock_report">
         ...
         buttons: [
            {
               ...
               title: 'Akinjide Store Stock Report'
            },
            {
               extend: 'print',
               exportOptions: {
