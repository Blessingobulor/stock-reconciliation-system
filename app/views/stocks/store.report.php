...
// delete from admin_transfer

        $result_admin_transfer = $db->query("DELETE FROM admin_transfer WHERE stock_name IN(SELECT stock_name FROM stock where id = '$delete_id' AND created_by = '$user_id')");
        ...
        // delete from akinjide_store_stock

        $result_akinjide_store_stock = $db->query("DELETE FROM akinjide_store_stock WHERE stock_name IN(SELECT stock_name FROM stock where id = '$delete_id' AND created_by = '$user_id')");


// delete from branch_sales

        $result_branch_sales = $db->query("DELETE FROM branch_sales WHERE stock_name IN(SELECT stock_name FROM stock where id = '$delete_id' AND created_by = '$user_id')");
        


// delete from central_store_recieve

        $result_central_store_recieve = $db->query("DELETE FROM central_store_recieve WHERE stock_name IN(SELECT stock_name FROM stock where id = '$delete_id' AND created_by = '$user_id')");
...
// delete from central_store_stock
        ...
        // delete from electrical_recieved_from_supplier
        $result_electrical_recieved_from_supplier = $db->query("DELETE FROM electrical_recieved_from_supplier WHERE stock_name IN(SELECT stock_name FROM stock where id = '$delete_id' AND created_by = '$user_id')");
        
        
        // delete from electrical_store_stock
        $result_electrical_store_stock = $db->query("DELETE FROM electrical_store_stock WHERE stock_name IN(SELECT stock_name FROM stock where id = '$delete_id' AND created_by = '$user_id')");
        
        
        
        // delete from electrical_store_transfer_to_branch
        $result_electrical_store_transfer_to_branch = $db->query("DELETE FROM electrical_store_transfer_to_branch WHERE stock_name IN(SELECT stock_name FROM stock where id = '$delete_id' AND created_by = '$user_id')");


 // delete from recieve_akinjide
        $result_recieve_akinjide = $db->query("DELETE FROM recieve_akinjide WHERE stock_name IN(SELECT stock_name FROM stock where id = '$delete_id' AND created_by = '$user_id')");
        
        
         // delete from recieve_from_branch
        $result_recieve_from_branch = $db->query("DELETE FROM recieve_from_branch WHERE stock_name IN(SELECT stock_name FROM stock where id = '$delete_id' AND created_by = '$user_id')");

  // delete from recieve_from_central_store
        $result_recieve_from_central_store = $db->query("DELETE FROM recieve_from_central_store WHERE stock_name IN(SELECT stock_name FROM stock where id = '$delete_id' AND created_by = '$user_id')");
        
        
          // delete from recieved_from_supplier
        $result_recieved_from_supplier = $db->query("DELETE FROM recieve_from_central_store WHERE stock_name IN(SELECT stock_name FROM stock where id = '$delete_id' AND created_by = '$user_id')");
        
        // delete from return_inward
        $result_return_inward = $db->query("DELETE FROM return_inward WHERE stock_name IN(SELECT stock_name FROM stock where id = '$delete_id' AND created_by = '$user_id')");
        
        
          // delete from return_stock
        $result_return_stock = $db->query("DELETE FROM return_stock WHERE stock_name IN(SELECT stock_name FROM stock where id = '$delete_id' AND created_by = '$user_id')");
    ...
    if ($result_stock && $result_recieve_from_central_store && $result_branch_sales && $result_recieve_from_branch && $result_return_stock && $result_transfer_to_branch && $result_recieved_from_supplier && $result_central_store_transfer_to_branch && $result_central_store_stock && $result_return_inward && $result_admin_transfer && $result_akinjide_sales && $result_akinjide_store_stock && $result_branch_sales && $result_central_store_recieve && $result_central_store_stock && $result_central_store_transfer && $result_central_store_transfer_to_branch && $result_electrical_recieved_from_supplier && $result_electrical_store_stock && $result_electrical_store_transfer_to_branch && $result_recieve_akinjide && $result_recieve_from_branch && $result_recieve_from_central_store && $result_recieved_from_supplier && $result_return_inward && $result_return_stock) {
