...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_stock'])) {
    ...
    // Delete the stock item from the recieved_from_supplier store table

   $db->query("DELETE FROM recieved_from_supplier WHERE id = $id");
...
<div>
    ...
    <h2>CENTRAL STORE RECEIVED FROM SUPPLIER REPORT</h2>
    <br><br>
    <hr>
    ...
    <a href="index.php?pg=recieved-from-supplier-new">
       <button style="margin: 30px; background-color: blue; max-width: 70px; height: 40px; position: grid; top: 30px; width: 70px; border-radius: 2px;" type="button" class="btn btn-primary"> Cancel </button>
        ...
        <button style="margin: 30px; max-width: 70px; height: 40px; position: grid; top: 30px; width: 70px; border-radius: 2px;"
            class="btn btn-primary btn-sm col" name="generate_report_recieved_supplier" type="submit"> Generate</button>
    </a>
         ...
         buttons: [
            {
               ...
               title: 'Central Store Recieved From Supplier Report'
            },
            {
               extend: 'print',
               exportOptions: {
