...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_stock'])) {
   ...
   $db->query("DELETE FROM central_store_recieve WHERE id = $id");
...
<div>
    ...
    <form style="margin: 40px;"class="row float-end <?= !Auth::access('cashier') ?> ">
            <h2>CENTRAL STORE STOCK RECIEVED REPORT</h2>
        ...
        <br><br>
                ...
                <a href="index.php?pg=central-store-recieve-new">
                ...
                <button style="margin: 30px; max-width: 70px; height: 40px; position: grid; top: 30px; width: 70px; border-ra dius: 2px;"
            class="btn btn-primary btn-sm col" name="generate_report_central_store_recieve" type="submit"> Generate</button>

            </a>

        <input type="hidden" name="pg" value="user-central-store-recieve-report">
        <input type="hidden" name="tab" value="central_store_recieve_report">
    ...
    <table class="table table-striped table-hover reportTable">
        ...
        <tbody>
            ...
            <?php  foreach($report as $report): ?>
                <tr>
                    <td><?=$report['recieve_stock_id']?></td>
                    <td><?=$report['branch_name']?></td>
                    <td><?=$report['source']?></td>
                    <td><?= date('j M, Y', strtotime($report['date'])) ?></td>
                    <td><?=$report['recieved_by']?></td>
                    <td><?=$report['name_of_reciever']?></td>
         ...
         buttons: [
            {
               ...
               title: 'Central Store Recieve Report'
            },
            {
               extend: 'print',
               exportOptions: {
