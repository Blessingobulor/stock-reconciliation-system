...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_stock'])) {
   ...
   $db->query("DELETE FROM central_store_recieve_from_branch WHERE id = $id");
...
<div>
    ...
    <h2>CENTRAL STORE RECIEVE FROM BRANCH REPORT</h2>
    <br><br>
    <hr>
    ...
    <a href="index.php?pg=central-store-recieve-from-branch-new">
       <button style="margin: 30px; background-color: blue; max-width: 70px; height: 40px; position: grid; top: 30px; width: 70px; border-radius: 2px;" type="button" class="btn btn-primary"> Cancel </button>
        ...
        <button style="margin: 30px; max-width: 70px; height: 40px; position: grid; top: 30px; width: 70px; border-radius: 2px;"
            class="btn btn-primary btn-sm col" name="generate_report_from_central_store_recieve_branch" type="submit"> Generate</button>
    </a>

    <input type="hidden" name="pg" value="user-central-store-recieve-from-branch-report">
    <input type="hidden" name="tab" value="central_store_recieve_from_branch_report">
	...
	<table class="table table-striped table-hover reportTable">
        ...
        <thead>
            <tr>
                ...
                <th>Source</th>
                <th>Date</th>
                <th>Reasons for recieving Of Stock</th>
                <th>Stock Name</th>
                <th>qty</th>
                <th>Category</th>
        ...
        <tbody>
            ...
            <?php  foreach($report as $report): ?>
                <tr>
                    ...
                    <td><?=$report['reasons_for_recieving_of_stock']?></td>
         ...
         buttons: [
            {
               ...
               title: 'Akinjide Stock Recieved Report'
            },
            {
               extend: 'print',
               exportOptions: {
